...
export default function GestionStockPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [worksData, movementsData, alertsData, statsData, pendingData, disciplinesData] = await Promise.all([
        ...
        apiClient.getStockAlertsWithFilters('alerts'),
  ...
  const loadAlertRules = async () => {
    try {
      const rules = await apiClient.getStockAlertsWithFilters('rules')
      setAlertRules(rules as any[])
    } catch (error: any) {
                ...
                <div className="space-y-3">
                  {stockAlerts.slice(0, 5).map((alert) => (
                    <div key={alert.id} className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        <AlertCircle className={`h-4 w-4 ${alert.severity === 'HIGH' ? 'text-red-500' :
                  ...
                  {currentWorks.length === 0 ? (
                    ...
                    currentWorks.map((work) => (
                      <TableRow key={work.id}>
                        ...
                        <TableCell>
                          <span className="font-medium text-sm">{work.price.toFixed(2)} FCFA</span>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <span className={`font-semibold ${work.stock === 0 ? 'text-red-600' :
