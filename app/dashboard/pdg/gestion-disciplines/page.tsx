...
export default function GestionDisciplinesPage() {
  ...
  const handleCreateDiscipline = async (disciplineData: any) => {
    try {
      const newDiscipline = await apiClient.createDiscipline(disciplineData) as Discipline
  ...
  const handleUpdateDiscipline = async (disciplineId: string, updates: any) => {
    try {
      const updatedDiscipline = await apiClient.updateDiscipline(disciplineId, updates) as Partial<Discipline>
      setDisciplines(prev =>
        prev.map(d =>
          d.id === disciplineId
            ? { ...d, ...updatedDiscipline }
          ...
          {filteredDisciplines.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredDisciplines.map((discipline) => (
                <Card key={discipline.id} className="hover:shadow-md transition-shadow flex flex-col">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                  ...
                  <CardContent className="flex-1 flex flex-col">
                          ...
                          <div>
                            ...
                            <div className="space-y-1">
                              {concepteurs
                                ...
                                .slice(0, 2)
                                .map(concepteur => (
                                  <div key={concepteur.id} className="text-sm text-muted-foreground truncate">
                                    • {concepteur.name}
                                  </div>
                                ))}
                              {(discipline.concepteurCount || 0) > 2 && (
                                <div className="text-sm text-muted-foreground">
                                  +{(discipline.concepteurCount || 0) - 2} autres...
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="space-y-1">
                              {auteurs
                                ...
                                .slice(0, 2)
                                .map(auteur => (
                                  <div key={auteur.id} className="text-sm text-muted-foreground truncate">
                                    • {auteur.name}
                                  </div>
                                ))}
                              {(discipline.auteurCount || 0) > 2 && (
                                <div className="text-sm text-muted-foreground">
                                  +{(discipline.auteurCount || 0) - 2} autres...
                                </div>
                              )}
                            </div>
                          </div>
                    ...
                    {/* Actions - Toujours en bas avec disposition fixe */}
                    <div className="grid grid-cols-2 gap-2 pt-4 border-t mt-4">
                      {/* Ligne 1 : Auteurs et Modifier */}
                      <Button
                        variant="outline"
                        size="sm"
                      ...
                      {/* Ligne 2 : Activer/Désactiver et Supprimer */}
                      <Button
                        variant="outline"
                        size="sm"
