...
export default function MessageriePage() {
  ...
  const loadConversations = async () => {
    try {
      setIsLoading(true)
      const response = await fetch('/api/auteur/messages')
  ...
  const loadMessages = async (conversationId: string) => {
    try {
      const response = await fetch(`/api/auteur/messages?conversationId=${conversationId}`)
      if (!response.ok) throw new Error('Erreur lors du chargement')
      const data = await response.json()
  ...
  const loadRecipients = async () => {
    try {
      const response = await fetch('/api/auteur/recipients')
      if (!response.ok) throw new Error('Erreur lors du chargement')
      const data = await response.json()
    ...
    <div className="p-6 h-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600">Communiquez avec le PDG et votre représentant</p>
        </div>
                ...
                <CardContent className="flex-1 flex flex-col p-0">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {messages.map((message) => (
                      <div
                        key={message.id}
                        className={`flex ${message.sender.role === 'AUTEUR' ? 'justify-end' : 'justify-start'}`}
                      >
                        <div
                          className={`max-w-[70%] p-3 rounded-lg ${
                            message.sender.role === 'AUTEUR'
      ...
      <Dialog open={showNewMessageModal} onOpenChange={setShowNewMessageModal}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Envoyer un message au PDG ou à votre représentant
            </DialogDescription>
          </DialogHeader>
