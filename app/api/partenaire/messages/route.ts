...
// GET /api/partenaire/messages - Récupérer les messages du partenaire
export async function GET(request: NextRequest) {
  try {
    ...
    if (conversationId) {
      // Récupérer les messages d'une conversation spécifique avec le PDG
    ...
    // Récupérer tous les messages du partenaire groupés par conversation
    const messages = await prisma.message.findMany({
      where: {
        OR: [
...
// POST /api/partenaire/messages - Envoyer un message
export async function POST(request: NextRequest) {
  try {
    ...
    // Vérifier que le destinataire existe et est autorisé (PDG uniquement)
    ...
    // Les partenaires ne peuvent envoyer des messages qu'au PDG
    if (recipient.role !== 'PDG') {
      return NextResponse.json({ 
        error: 'Vous ne pouvez envoyer des messages qu\'au PDG' 
      }, { status: 403 })
    }
